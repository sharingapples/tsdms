...
class Application extends React.Component {
  _fixPath(path) {
    ...
    return path.replace(/:([a-z_A-Z0-9]*)/g, (match, p1) => this.props.params[p1]);
  }

  render() {
    // The root path for application (should be '/app/')
    const rootPath = this._fixPath(this.props.route.path);

    // Also detect the selected route
    const topLevelRoute = this.props.routes[1];
    const selected = topLevelRoute.indexRoute ? topLevelRoute.indexRoute.component
          : topLevelRoute.component;
        ...
        <div className="main">
          ...
          <div className="content">
            <div className="top">
              <TopBar user={user}/>
              <ContentNavigation
                  pathPrefix={rootPath + this._fixPath(topLevelRoute.path)}
                  root={selected}
                  selected={this.props.routes[this.props.routes.length - 1].component}/>
            </div>
